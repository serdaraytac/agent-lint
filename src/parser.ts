...
const FILENAME_PLATFORM_MAP: Record<string, Platform> = {
  "claude.md": "claude",
...
export function detectPlatform(filename: string): Platform {
  ...
  if (basename.includes("cursor")) return "cursor";
  if (basename.includes("cline")) return "cline";
  ...
  if (basename.includes("amp")) return "amp";
  if (basename.includes("kimi")) return "kimi";
  if (basename.includes("warp")) return "warp";
...
export function parseConfig(filename: string, content: string): ParsedConfig {
