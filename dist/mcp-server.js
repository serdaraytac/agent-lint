...
const server = new Server({ name: "agent-lint", version: "1.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "analyze_config",
            description: "Analyze an AI coding agent config file and return a score with categorized issues. " +
        ...
        },
        {
            ...
            description: "Analyze an AI coding agent config file and return an optimized version with a before/after score. " +
        ...
        {
            name: "scan_project",
            description: "Scan a project directory and return all recognized AI coding agent config files found, " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "optimize_config") {
        ...
        const output = {
            ...
            ...(safeArgs.write === true && { writtenTo: resolve(safeArgs.filepath) }),
        };
        return {
            content: [{ type: "text", text: JSON.stringify(output, null, 2) }],
        };
